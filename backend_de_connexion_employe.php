...
if (isset($_POST["mail"]) && isset($_POST["mot_de_passe"]) ){
  ...
  $verificateur = new verification_de_ce_formulaire("formulaire_de_connexion_employe") ;//creation de l objet de cette classe en question
  ...
  $simplificateur = new Simplificateur_de_syntaxe("formulaire_de_connexion_employe") ; //creation d un objet qui va m aider a creer ma liste de presence
  
  $recherche_de_l_utilisateur = $ma_base_de_donnee->connexion->prepare("SELECT * FROM employes WHERE email = ? ; ") ; //recherche de l utilsateur dans ma requete preparer
  ...
  $mon_identifiant_unique = $ma_base_de_donnee->connexion->prepare("SELECT id_employe FROM employes WHERE email = ? ; ") ; //requete permettant de rechercher mon identifiant
  ...
  $tableau_de_mon_identifiant_unique =  $simplificateur->stocke_le_resultat_de_la_requete($mon_identifiant_unique,"id_employe") ;//ceci est une methode qui me permet de stocker les identifiants de tout ceux qui doivent reçevoir la liste
  ...
  if( $mot_de_passe_hacher == $mot_de_passe_de_la_BD){//comparaison de si mot de passe que l utilsateur entre est egale au mot de passe qui est dans ma base de donnée
    ...
    header("Location:formulaire_de_gestion_des_commandes.php"); //redirection vers la meme page
    ...
    header("Location:formulaire_de_connexion_employe.php"); //redirection vers la meme page
    exit();//fermeture de la redirection
