<?php  //ouverture de ma balise php
 ...
 if (isset($_POST["nom_utilisateur"])  && isset($_POST["mail"])  && isset($_POST["mot_de_passe"]) && isset($_POST["ma_confirmation_mot_de_passe"]) ){/////condition pour verifier si j ai envoyer mon formulaire

    ///ce bloc  me permet de récupérer  les informations sur les assistantes 
    $nom_utilisateur = $_POST["nom_utilisateur"];//recuperation du nom utilisateur
    ...
    $confirmation_du_mot_de_passe = $_POST["ma_confirmation_mot_de_passe"];//recuperation de la confirmation du mot de passe
     ...
     $recherche_email_dans_la_BD = $ma_base_de_donnee->connexion->prepare("SELECT email FROM employes ;");//requete sql recherchant les emails
     ...
     $verificateur = new verification_de_ce_formulaire("formulaire_d_administrateur_de_gestion_d_employe") ;//creation de l objet de cette classe en question
    ...
    $insertion_d_information = $ma_base_de_donnee->connexion->prepare("INSERT INTO employes(nom_employe,email,mot_de_passe) VALUES (?,?,?) ;") ;
    $insertion_d_information->bind_param("sss",  $nom_utilisateur , $email ,$mot_de_passe_hacher) ;  //remplissage de parametre
